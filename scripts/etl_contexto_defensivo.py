...
def destacados_def(equipo):
    df = pd.read_csv(SEASONSTATS[equipo])
    # Excluimos a los porteros (sus "recuperaciones" no son acción defensiva de campo)
    df = df[df["posicion"] != "Goalkeeper"]
    n = lambda c: pd.to_numeric(df[c], errors="coerce").fillna(0)
    out = pd.DataFrame({
        "jugador": df["nombre"],
        "entradas": n("Total Tackles").astype(int),
        "intercepciones": n("Interceptions").astype(int),
        "recuperaciones": n("Recoveries").astype(int),
    })
    out["acciones_def"] = out["entradas"] + out["intercepciones"] + out["recuperaciones"]
    return out.sort_values("acciones_def", ascending=False).head(6)
...
def main():
    ...
    for eq in EQUIPOS:
        ...
        destacados_def(eq).to_csv(OUT / f"destacados_def_{eq.lower()}.csv", index=False, sep=";", decimal=",", encoding="utf-8-sig")
        fila = tabla.loc[eq]
